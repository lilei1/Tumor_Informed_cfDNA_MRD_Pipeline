...
process INTEGRATE_FEATURES {
    ...
    input:
    tuple val(sample), val(timepoint), path(variant_evidence), path(fragmentomics), path(endmotifs), path(tss_coverage), path(cnv_results), path(error_model_json)
    path truthset_bed
    path ref_genome
...
process CALCULATE_LONGITUDINAL_MRD {
    ...
    def analyze_longitudinal_mrd(mrd_files):
        ...
        if len(combined_df) > 1:
            ...
            y = combined_df['mrd_probability'].values
...
process GENERATE_MRD_REPORT {
    ...
    output:
    path "*.mrd_report.html", emit: mrd_report
    path "*.mrd_report.pdf", emit: mrd_report_pdf
    path "*.log", emit: logs
    ...
    def generate_html_report():
        """Generate comprehensive HTML MRD report"""
        ...
        <html>
        <head>
            <title>MRD Analysis Report</title>
            <style>
                ...
                .feature-table th { background-color: #f5f5f5; }
                .mrd-score { font-size: 24px; font-weight: bold; color: #007bff; }
        ...
        <body>
            <div class="header">
                <h1>Minimal Residual Disease (MRD) Analysis Report</h1>
                <p>Generated on: """ + str(datetime.now()) + """</p>
                <p>Pipeline: Tumor-Informed cfDNA MRD Pipeline v1.0</p>
            </div>
        """
        
        # Add MRD scores section
        html_content += """
            <div class="section">
                <h2>MRD Assessment Results</h2>
                <p>This report presents the integrated analysis of multiple molecular features to assess MRD status.</p>
            </div>
        """
        ...
        html_content += """
            <div class="section">
                ...
                <p>The relative importance of different molecular features in MRD assessment:</p>
            </div>
        """
        
        # Add longitudinal analysis section
        html_content += """
            <div class="section">
                <h2>Longitudinal MRD Trends</h2>
                <p>Analysis of MRD dynamics over time:</p>
            </div>
        """
        ...
        html_content += """
            <div class="section">
                ...
                <p><strong>Error Model:</strong> Context-aware error suppression using healthy donor background</p>
                <p><strong>Feature Integration:</strong> Multi-modal approach combining variant, fragmentomics, and CNV data</p>
                <p><strong>Scoring Method:</strong> Weighted feature combination with error-aware variant calling</p>
            </div>
        """
        ...
        # Save HTML report
        with open('mrd_analysis_report.html', 'w') as f:
    ...
    if command -v wkhtmltopdf >/dev/null 2>&1; then
        wkhtmltopdf mrd_analysis_report.html mrd_analysis_report.pdf
        echo "PDF report generated successfully"
    else
        echo "wkhtmltopdf not available, PDF generation skipped"
        # Create a simple text version
        echo "MRD Analysis Report" > mrd_analysis_report.pdf
        echo "Generated on $(date)" >> mrd_analysis_report.pdf
        echo "PDF conversion not available" >> mrd_analysis_report.pdf
    fi
